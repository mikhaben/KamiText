...
fn table_single_kind() {
    ...
    e.set_selection(ByteRange::new(0, 0)).unwrap();
    common::assert_invariants(&e);
    // The table node range includes its trailing newline (same convention as
    // headings; probed pulldown behavior).
    assert_eq!(
        segs(&e),
        vec!["0..2 u16:0..2 BODY", "2..32 u16:2..32 TABLE"]
    );
}
...
fn thematic_break() {
    let e = engine("---\n");
    common::assert_invariants(&e);
    assert_eq!(segs(&e), vec!["0..3 u16:0..3 HR", "3..4 u16:3..4 BODY"]);
}
